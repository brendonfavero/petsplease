...
//MySQL "strict mode" - set to 1 if you have MySQL 5.0 or higher, that
// runs in "strict mode".  If you are not sure, change this to 1 if
// the software has problems caused by database errors.
$strict_mode = 0;
...
//WARNING:  Only 'mysql' database is fully tested, other database types may 
// not work properly.
$db_type ='mysql';
...
/*
 ...
 Most of these are automatically detected, usually only the first setting
 needs to be modified, and even then only if changing the admin directory
 name to something other than "admin/".  The rest of the settings you will 
 need to un-comment to modify.  Note that changing the actual folder name,
 besides changing the admin folder name, is not fully supported.
*/
...
//The URL of the main templates directory, this should coincide with
//the GEO_TEMPLATE_DIR.  This must be a relative URL path, relative to the
//installation.  If using absolute path, things may not work correctly in admin
//panel, since certain files are shared by client and admin side.
//By default this is set to geo_templates/ which should work for
//all sites unless you intend to change the folder name.
#define ('GEO_TEMPLATE_URL','relative/path/to/geo_templates/');
