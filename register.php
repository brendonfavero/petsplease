...
if (strlen($register->debug_email) > 0) $register->sendMail($register->debug_email,'DEBUG '.substr($register->uniqueTimeStamp(),-6),'LINE = '.__LINE__,'FILE = register.php');
if ($user_id !== 0)
{
	if (strlen($register->debug_email) > 0) $register->sendMail($register->debug_email,'DEBUG '.substr($register->uniqueTimeStamp(),-6),'LINE = '.__LINE__,'FILE = register.php');
...
elseif (isset($_REQUEST['b'])&&$_REQUEST["b"] == 5)
{
	if (strlen($register->debug_email) > 0) $register->sendMail($register->debug_email,'DEBUG '.substr($register->uniqueTimeStamp(),-6),'LINE = '.__LINE__,'FILE = register.php');
	//reset filter
	$register->update_filter_id(0);
	if ($register->check_filter())
		$register->filter_select($_REQUEST["registration_filter_id"]);
	else
	...
	elseif (!$register->personal_info_check)
	{
		if (isset($_POST['c'])&&$_POST["c"])
		{
			...
			elseif ($register->check_info($_POST["c"]))
			{
				...
				$register->set_new_user_id_in_current_session();
				if ($db->get_site_setting('use_email_verification_at_registration') ||
