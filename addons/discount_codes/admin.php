...
class addon_discount_codes_admin extends addon_discount_codes_info {
	...
	public function init_pages ()
	{
		...
		menu_page::addonAddPage('discounts_view','','View Discount Codes','discount_codes',$this->icon_image);
		menu_page::addonAddPage('discounts_new','','New Discount Code','discount_codes',$this->icon_image);
		menu_page::addonAddPage('discounts_edit','discounts_view','Edit Discount Code','discount_codes',$this->icon_image,'sub_page');
		menu_page::addonAddPage('discounts_delete','discounts_view','Delete Discount Code','discount_codes',$this->icon_image,'sub_page');
		menu_page::addonAddPage('discounts_stats','discounts_view','Discount Code Stats','discount_codes',$this->icon_image,'sub_page');
	}
	...
	public function update_discounts_edit ()
	{
		...
		$duplicates = $db->GetOne("SELECT COUNT(*) FROM ".self::DISCOUNT_TABLE." WHERE `discount_id`!=$discount_id AND `discount_code`=?",
		...
		} else if ($settings['is_group_specific']) {
		...
		if ($settings['is_group_specific'] && $discount_id) {
			...
			foreach ($groups as $groupId) {
				$db->Execute("INSERT INTO ".self::DISCOUNT_GROUPS_TABLE." SET `discount_id`=?, `group_id`=?",
	...
	public function display_discounts_stats ()
	{
		...
		$tpl_vars['data'] = $data = $db->GetRow("SELECT * FROM ".self::DISCOUNT_TABLE." WHERE `discount_id`=?",
			array($discount_id));
		...
		if ($data['discount_id']) {
			...
			if ($data['is_group_specific']) {
				//group specific
				$groups_all = $db->GetAll("SELECT `group_id` FROM ".self::DISCOUNT_GROUPS_TABLE." WHERE `discount_id`=?",
