...
for this bridge.  Debug info, '.$this->db->ErrorMsg(),true);
	...
	function update_current_userdata() {
		...
		if (($this->get_setting('USE_FILTERS')) && ($this->get_setting('OPTIONAL_1_FILTER_ASSOCIATION')))
			$this->user_info["optional_field_1"] = $this->old_user_data->OPTIONAL_FIELD_1;
		if (($this->get_setting('USE_FILTERS')) && ($this->get_setting('OPTIONAL_2_FILTER_ASSOCIATION')))
			$this->user_info["optional_field_2"] = $this->old_user_data->OPTIONAL_FIELD_2;
		if (($this->get_setting('USE_FILTERS')) && ($this->get_setting('OPTIONAL_3_FILTER_ASSOCIATION')))
			$this->user_info["optional_field_3"] = $this->old_user_data->OPTIONAL_FIELD_3;
		if (($this->get_setting('USE_FILTERS')) && ($this->get_setting('OPTIONAL_4_FILTER_ASSOCIATION')))
			$this->user_info["optional_field_4"] = $this->old_user_data->OPTIONAL_FIELD_4;
		if (($this->get_setting('USE_FILTERS')) && ($this->get_setting('OPTIONAL_5_FILTER_ASSOCIATION')))
			$this->user_info["optional_field_5"] = $this->old_user_data->OPTIONAL_FIELD_5;
		if (($this->get_setting('USE_FILTERS')) && ($this->get_setting('OPTIONAL_6_FILTER_ASSOCIATION')))
			$this->user_info["optional_field_6"] = $this->old_user_data->OPTIONAL_FIELD_6;
		if (($this->get_setting('USE_FILTERS')) && ($this->get_setting('OPTIONAL_7_FILTER_ASSOCIATION')))
			$this->user_info["optional_field_7"] = $this->old_user_data->OPTIONAL_FIELD_7;
		if (($this->get_setting('USE_FILTERS')) && ($this->get_setting('OPTIONAL_8_FILTER_ASSOCIATION')))
			$this->user_info["optional_field_8"] = $this->old_user_data->OPTIONAL_FIELD_8;
		if (($this->get_setting('USE_FILTERS')) && ($this->get_setting('OPTIONAL_9_FILTER_ASSOCIATION')))
			$this->user_info["optional_field_9"] = $this->old_user_data->OPTIONAL_FIELD_9;
		if (($this->get_setting('USE_FILTERS')) && ($this->get_setting('OPTIONAL_10_FILTER_ASSOCIATION')))
	...
	function insert_into_userdata_table() {
		...
		$this->user_info['registration_filter_id'] = (isset($this->user_info['registration_filter_id']))? $this->user_info['registration_filter_id']: 0;
		if (!isset($this->user_info['registration_filter_id'])){
			$this->user_info['registration_filter_id'] = 0;//filter id needs to be 0
		}
		$query_data=array(
			$this->user_info['id'],
			...
			$this->user_info['registration_filter_id']);
	...
	function insert_credits_upon_registration()
	{
		...
		} else if ($this->user_info['price_plan']['credits_expire_type'] == 2) {
	...
	function connect(){
		...
		if (!$this->db->isConnected()){
			$this->log("Error connecting to bridge database, check settings for this bridge.  DB Settings used: host:{$this->settings['db_host']} usr:{$this->settings['db_user']} pass:{$this->settings['db_pass']} database:{$this->settings['db_name']}, DB error returned: ".$this->db->ErrorMsg(),true);//log it
			return false;
		}
