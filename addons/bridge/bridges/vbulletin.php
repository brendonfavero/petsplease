...
class bridge_vbulletin extends addon_bridge_util {
	...
	var $setting_desc = array (
		...
		'license_key' => array (
				...
				'desc' => 'The license key you were given when you bought your vBulletin license.<br /><br />The key can be found at the top of one of your vBulletin files.'
			)
	...
	function bridge_vbulletin(){
		$this->setting_desc['config_path']['desc'] .=  '
			<br /><br />Path to this installation\'s root: <em>'.GEO_BASE_DIR.'</em>';
	}
	...
	function connect(){
		...
		if (!$this->db->isConnected()){
			...
			DB Settings used: host:{$host} usr:{$user} pass:{$pass} database:{$db_name}, DB error returned: ".$this->db->ErrorMsg(), true);//log it
			return false;
		}
