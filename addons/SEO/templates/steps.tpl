...
<div class="breadcrumbBorder">
	...
	{foreach $steps as $step_number => $this_step}
		<li {if $step_number == $smarty.get.step}class="current"{/if}>{$this_step}</li>
	{/foreach}
	</ul>
</div>
<br />
...
{if $go_step}
	{if $go_step eq 1}
		...
		<br /><br />
		{include g_type="admin" file="HTML/add_button.tpl" link="?mc=addon_cat_SEO&page=addon_SEO_main_config&step=2" label="Continue"}
		 <span style="font-weight: bold;">OR</span> 
		{include g_type="admin" file="HTML/add_button.tpl" link="onclick=\"$('customUrls').toggle();\"" link_is_really_javascript=1 label="Customize URLs"}
		<div id="customUrls">
		<br />
