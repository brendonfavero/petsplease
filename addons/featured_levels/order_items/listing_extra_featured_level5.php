<?php
//listing_extra_featured_level5.php
/**************************************************************************
...
class listing_extra_featured_level5OrderItem extends geoOrderItem {
	var $defaultProcessOrder = 39;
	protected $type = 'listing_extra_featured_level5';
	const type = 'listing_extra_featured_level5';
	/**
	...
	public static function geoCart_initSteps_addOtherDetails(){
		...
		if (!$db->get_site_setting('use_featured_feature_5')) {
	...
	public static function geoCart_other_detailsCheckVars($c_var = false){
		...
		if (isset($_POST['c']) || $c_var){
			$use_featured_ad_5 = ((isset($_POST['c']['featured_ad_5']) && $_POST['c']['featured_ad_5'])? 1: 0);
			$use_featured_ad_5 = (($use_featured_ad_5 || (isset($c_var['featured_ad_5']) && $c_var['featured_ad_5']))? 1: 0);
			
			$cart->site->session_variables['featured_ad_5'] = $use_featured_ad_5;
			...
			//get current attached featured_ad_5, if exists..
			$items = $cart->order->getItem('listing_extra_featured_level5');
			$order_item = false;
			if (is_array($items)){
			...
			if (!$use_featured_ad_5){
				...
				if (!$order_item){
					$order_item = new listing_extra_featured_level5OrderItem;
					...
					trigger_error('DEBUG CART: Adding featured_level5: <pre>'.print_r($order_item,1).'</pre>');
				} else {
					trigger_error('DEBUG CART: featured_ad_5 already attached: <pre>'.print_r($order_item,1).'</pre>');
					$cart->order->addItem($order_item);
				}
				//get the price for featured_ad_5
				$cost = (!geoMaster::is('site_fees'))? 0: $cart->price_plan['featured_ad_price_5'];
				$order_item->setCost($cost);
				$order_item->setCreated($cart->order->getCreated());
				
				//set details specific to featured_ad_5
			...
			trigger_error('DEBUG CART: featured_ad_5: '.$cart->site->session_variables['featured_ad_5']);
		}
		
		//but children might, get steps from children as well.
		$children = geoOrderItem::getChildrenTypes('listing_extra_featured_level5');
		foreach ($children as $child){
			geoOrderItem::callUpdate('geoCart_other_detailsCheckVars',null,$child);
		}
	}
	...
	public function getDisplayDetails ($inCart,$inEmail=false)
	{
		...
		if ($parent && $parent->getType() === 'listing_renew_upgrade') {
			$return = $parent->checkNoDowngrade($return, 'featured_ad_5');
		}
		return $return;
	}
	...
	public static function geoCart_other_detailsDisplay (){
		$cart = geoCart::getInstance();
		if (!in_array ($cart->main_type,geoOrderItem::getParentTypesFor(self::type)) || !geoPC::is_ent() || !$cart->db->get_site_setting('use_featured_feature_5'))
		{
			//do not show thingy for bolding
			return '';
		}
		if (isset($cart->item->renew_upgrade) && $cart->item->renew_upgrade == listing_renew_upgradeOrderItem::upgrade){
			...
			if ($cart->site->parent_session_variables['featured_ad_5']){
				//already exists on parent, do not allow adding
				return '';
			}
		}
		$cart->setPricePlan($cart->item->getPricePlan(),$cart->item->getCategory());
		if (!$cart->price_plan['use_featured_ads_level_5']){
			//not used in price plan
			return '';
		}
		$price = (!geoMaster::is('site_fees'))? 0: $cart->price_plan['featured_ad_price_5'];
		$return = array (
			'checkbox_name' => 'c[featured_ad_5]', //no checkbox display
			'title' => geoString::fromDB($cart->site->messages[2266]),
			'display_help_link' => $cart->site->display_help_link(2267),//if 0, will display no help icon thingy
			'price_display' => geoString::displayPrice($price, false, false, 'cart'),
			'checked' => (($cart->site->session_variables['featured_ad_5']) ? "checked=\"checked\" " : "")
		);
		if ($cart->item && $cart->item->getType()==='listing_renew_upgrade') {
			...
			$return = $cart->item->checkNoDowngrade($return, 'featured_ad_5');
		}
		return $return;
	}
	...
	public static function copyListing(){
		...
		if($cart->site->session_variables['featured_ad_5'])
		{
			if (!in_array ($cart->main_type,geoOrderItem::getParentTypesFor(self::type)) || !$cart->db->get_site_setting('use_featured_ad_5_feature'))
			{
				//do not show
				return '';
			}
			$cart->setPricePlan($cart->item->getPricePlan(),$cart->item->getCategory());
			if (geoPC::is_ent() && !$cart->price_plan['use_featured_ad_5']){
