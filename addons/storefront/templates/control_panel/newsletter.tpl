	...
	{if $show_newsletter}
		...
		<form method="post" action="{$classifieds_file_name}?a=ap&amp;addon=storefront&amp;page=control_panel&amp;action=update&amp;action_type=newsletter">
			<div class="content_box">
				<h2 class="title">{$msgs.usercp_news_settings_header}</h2>
		...
		<form method="post" action="{$classifieds_file_name}?a=ap&amp;addon=storefront&amp;page=control_panel&amp;action=update&amp;action_type=newsletter">
			<div class="content_box">
				<h1 class="title">{$msgs.usercp_news_send_header}</h1>
				<div class="{cycle values='row_odd,row_even'}">
					<label class="field_label">
				...
				<h1 class="subtitle">
					{$msgs.usercp_news_send_bodyheader}
				</h1>
				<div class="{cycle values='row_odd,row_even'}">
					<a href="javascript:void(0)" onclick="geoWysiwyg.toggleTinyEditors();">{$messages.add_remove_wysiwyg}</a>
				</div>
				<div class="{cycle values='row_odd,row_even'}">
					<textarea class="editor field" name="data[newsletter_body]" id="newsletter" style="width: 98%; height: 200px;"></textarea>		
				</div>
				<div class="center">
