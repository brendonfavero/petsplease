<?php
//addons/sharing/methods/linkedin.php
/**************************************************************************
...
class addon_sharing_method_linkedin {
	...
	public function getMethodsForListing($listingId) {
		...
		return $msgs['method_btn_linkedin'];
	...
	{
		$tpl = new geoTemplate('addon','sharing');
		$tpl->assign('iconUrl', geoTemplate::getUrl('images','addon/sharing/icon_linkedin.png')); 
		$msgs = geoAddon::getText('geo_addons','sharing'); 
		$tpl->assign('text', $msgs['shortlink_linkedin']);
		...
		$tpl->assign('link', 'http://www.linkedin.com/cws/share?url='.$urlToListing);
