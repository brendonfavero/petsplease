...
<div class="box_head_1_dyn imageBoxTitle{if $slotData.empty}Empty{/if}{if $position==1} firstUploadSlotTitle{/if}" id="adimageTitle{$position}">
	{if $slotData.empty}
		{$messages.500695}
	{else}
		<div class="imageBoxTitleButtons">
			<span class="deleteImage" id="addeleteImage_{$position}" onclick="geoUHExtra.deleteImage({$position});">
				{if $in_admin}
		...
		<div class="imageBoxTitleButtons">
			<span class="editImage" id="adeditImage_{$position}" onclick="geoUHExtra.editImage({$position});">
				{if $in_admin}
...
<div class="imagePreview{if $slotData.empty} emptyPreview{/if}" id="adimagePreview_{$position}">
	<div>
		{if $slotData.empty}
			{$messages.500697}
...
<input type="hidden" id="adimageTitle_{$position}" value="{$slotData.image.image_text}" />
<br />
{if $slotData.cost}
