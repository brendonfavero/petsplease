...
<div class="box_head_1_dyn imageBoxTitle{if $slotData.empty}Empty{/if}{if $position==1} firstUploadSlotTitle{/if}" id="adlogoTitle{$position}">
	{if $slotData.empty}
		{$messages.500695}
	{else}
		<div class="imageBoxTitleButtons">
			<span class="deleteImage" id="adlogodeleteImage_{$position}" onclick="geoUHLogo.deleteImage({$position});">
				{if $in_admin}
		...
		<div class="imageBoxTitleButtons">
			<span class="editImage" id="adlogoeditImage_{$position}" onclick="geoUHLogo.editImage({$position});">
				{if $in_admin}
...
<div class="imagePreview{if $slotData.empty} emptyPreview{/if}" id="adlogologoPreview_{$position}">
	<div>
		{if $slotData.empty}
			{$messages.500697}
...
<input type="hidden" id="adlogologoTitle_{$position}" value="{$slotData.image.image_text}" />
<br />
{if $slotData.cost}
