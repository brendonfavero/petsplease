			...
			<thead>
				<tr style="border-bottom: thin solid #000;">
					{foreach $combined_options as $group}
						<th style="padding-right: 1em;">{$group.label}</th>
					{/foreach}
						...
						<td>
							<select class="cost-options-combined-selection">
								{foreach $group.options as $option}
									<option value="{$option.comb_id}"{if $option@first} selected="selected"{/if}>{$option.label}</option>
								{/foreach}
							</select>
						</td>
					{/foreach}
					...
					<tr>
						{foreach $combined_options as $group}
							<td>
								<select class="cost-options-combined-selection">
									{foreach $group.options as $option}
										<option value="{$option.comb_id}"{if $hash|strstr:$option.comb_id} selected="selected"{/if}>{$option.label}</option>
									{/foreach}
								</select>
							</td>
						{/foreach}
