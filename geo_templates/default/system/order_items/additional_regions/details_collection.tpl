	...
	var gjAddRegions = {
		...
		updateButtons: function () {
			if (jQuery('.additional_use[value="0"]').length) {
				...
				jQuery('.additional_region_add').show('fast');
			} else {
				//none showing any more!
				jQuery('.additional_region_add').hide('fast');
			}
			//hide all of the remove buttons
			jQuery('.additional_region_remove').hide('fast');
			...
			jQuery('.additional_use').gj('filterValue', '1').filter(':last').closest('.additional_regions_wrapper')
				.find('.additional_region_remove').show('fast');
		},
