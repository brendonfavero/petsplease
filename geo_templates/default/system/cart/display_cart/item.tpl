...
<div class="{if $item.css_class}{$item.css_class}{else}cart_item{if $is_sub eq 1}_child{/if}{/if}">
	...
	{if !$view_only}
		<div class="cart_item_buttons">
			...
			{/if}
			{if $item.canPreview}
				<a href="{$cart_url}&amp;action=preview&amp;item={$k}" onclick="window.open(this.href,'previewWindow','scrollbars=yes,status=no,width=1066,height=600'); return false;" class="preview">{$messages.500262}</a>
			{/if}
		</div>
	{/if}
