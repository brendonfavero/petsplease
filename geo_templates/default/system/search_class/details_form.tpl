{* 7.2beta3-80-gb748642 *}

{$errors}

<div class="search_content_box">
	<h1 class="title">{$messages.571}</h1>
	<p class="page_instructions">{$messages.572} {$tooltip.2}</p>
...
<form action="{$classifieds_file_name}" method="get">
	<div>
		...
		<div class="search_content_box">
			...
			{/if}
			{if $messages.1442}
				<h1 class="subtitle">{$messages.1442}</h1>
			{/if}
			...
			{if $queryFields.price}
				...
				<div class="{cycle values='row_odd,row_even'}">
					<label class="field_label"></label>
					<label>{$messages.1440}<input id="by_price_lower" name='b[by_price_lower]' size='15' maxlength='15' class="field" /></label>
					<label>{$messages.1441}<input id="by_price_higher" name='b[by_price_higher]' size='15' maxlength='15' class="field" /></label>
				</div>
			{/if}
		</div>
	</div>
	...
	{if $queryFields.business_type}
		<div>
			<div class="search_content_box">
				<h1 class="subtitle">{$messages.1439}</h1>
				<div class="{cycle values='row_odd,row_even'}">
					<div>
						<label class="field_label"></label>
	...
	{if $region_selector || $use_zip_distance_calculator || $queryFields.city || $queryFields.zip || $show_optionals || $addonCriteria || $leveled_fields}
		...
		<div class="search_content_box">
			{if $use_zip_distance_calculator eq 1}
				<h1 class="subtitle">{$messages.1949}</h1>				
				<div class="{cycle values='row_odd,row_even'}">
