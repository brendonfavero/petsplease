...
<ul id="breadcrumb">
	<li class="element highlight">{$additional_text_12}</li>
	<li class="element">{listing tag='category_tree'}</li>
</ul>

<h1 class="listing_title" style="display: inline;">
	{$title}
	{if $minimum_bid}&nbsp;-&nbsp;<span class="value price">{$minimum_bid}</span>{/if}
...
<div class="listing_leftcol">
	<div class="content_box_1 cntr">
		...
		{if $minimum_label}
			...
			<p>{$reserve}</p>
		{/if}
		<p>{$viewed_count_label} {listing field='viewed_count'}</p>
	</div>
	<!-- SELLER INFO BEGIN -->
	<h2 class="title rounded_top">{$seller_label}</h2>
	
	<div class="content_box_2">
		<h1 class="seller_username">{listing tag='seller'} - {listing tag='seller_rating'}</h1>
		<p class="content_section">
			{$seller_rating_label}&nbsp;{listing tag='feedback_link'}&nbsp;&nbsp;{$seller_rating_scale_explanation}
		</p>
		<p class="content_section">
	...
	<div class="content_box_3 cntr">
		{* This is a good place to insert banner ads, optimized for width of
			160 px *}
...
<div class="listing_maincol">
	<h1 class="title" style="margin-bottom: 5px;">{$additional_text_12} {$additional_text_13}</h1>
	...
	<div class="content_box_1" style="margin-bottom:1px;">
		<div class="bid_box">
			{if $minimum_bid}
				<div class="bid_amount">
					{$minimum_label}<br />
					<span class="value price" style="font-size: 1.6em;">{$minimum_bid}</span><br />
				...
				<div class="bid_box_note">
					<span class="mini_note box_pad">{$additional_text_15}</span>
				</div>
			{/if}
			{if $buy_now_data}
				<div style="margin-top: 10px;">
					<div class="bid_amount">
						{$buy_now_label}<br />
						<span class="value price" style="font-size: 1.6em;">{$buy_now_data}</span><br />
						{listing tag='price_for_how_many'}
					</div>
					<div class="bid_button">
		...
		<ul class="info auction"> 
			...
			<li class="value">{$date_started}<br /> <span class="sub_note">({$bid_start_date_label} {$bid_start_date})</span></li>
		</ul>
		<div class="clr"></div>
	...
	{listing tag='image_block' assign='image_block'}
	{if $offsite_videos_block||$image_block}
