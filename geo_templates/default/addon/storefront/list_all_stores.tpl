...
{if $states}
	<div class="content_box {cycle values="row_even,row_odd"}">
		<form action="" method="post" id="ssf">
			...
			<select name="storefront_state_filter" id="storefront_state_filter" onchange="$('ssf').submit();" class="field">
		...
		<table border='0' cellpadding='3' cellspacing='1' width='100%'>
			...
			{foreach from=$stores item="store"}
				<tr class="{cycle values='row_even,row_odd'}">
					{if $switches.logo}<td class="nowrap center"><a href="{$classifieds_file_name}?a=ap&amp;addon=storefront&amp;page=home&amp;store={$store.userid}">{$store.image}</a></td>{/if}
					{if $switches.title}<td class="title"><span class="highlight_links"><a href="{$classifieds_file_name}?a=ap&amp;addon=storefront&amp;page=home&amp;store={$store.userid}"><strong>{$store.name}</strong></a></span> {if $switches.description}<p><a href="{$classifieds_file_name}?a=ap&amp;addon=storefront&amp;page=home&amp;store={$store.userid}">{$store.desc}</a></p>{/if}</td>{/if}
