...
{foreach from=$categories item=cats key=column name=catlist}
	<div class="category_column{if $column_css} {$column_css}{/if}" style="width: {$col_width};">
		<ul>
			{foreach from=$cats item=cat}
				<li class="element  category_{$cat.category_id}">
					<a href="{$link}{$cat.category_id}">
						{if $module.display_category_image && $cat.category_image}<img src="{external file=$cat.category_image}" alt="" />{/if}
...
{foreachelse}
	{if $module.display_no_subcategory_message}
		<div class="category_column" style="width: {$col_width};">
			<ul>
				<li class="element">
					<p>
						{$no_subcategory_text}
