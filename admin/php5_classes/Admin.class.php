 ...
 */
	 ...
	 * add any html needed in the header using getView()->header_html .= 'html',
	 ...
	 * @param string $extra_header_html No longer used
	 * @param string $additional_header_html html to be inserted in head tag
	 ...
	 */
	public static function display_page($body, $title = '', $image = '', $extra_header_html = '', $additional_header_html = '', $additional_body_tag_attributes = '') {
		$db = DataAccess::getInstance();
		$admin = geoAdmin::getInstance();
		$view = $admin->getView();
		$view->addBody($body);
		
		$view->_header_html .= $extra_header_html . $additional_header_html;
		...
		echo $admin->initViewDefault($page_name, $title, $image, $extra_header_html, $additional_header_html);
	}
	...
	public function load_page(){
		...
		if ($db->get_site_setting('adminLandingPage') == 'home') {
		...
		if (is_object($current_cat)){
			...
			if (isset($current_cat->title) && strlen($current_cat->title)){
				...
				$page_data->breadcrumb_title = $title.'<span> &gt; </span>'.$page_data->breadcrumb_title;
	 ...
	 */
	public function initViewDefault ($page_name = '', $title='', $image='', $extra_header_html='', $additional_header_html='')
	{
		$view = $this->getView();
		$db = DataAccess::getInstance();
		if (isset($page_name) && strlen($page_name) > 0){
		...
		if ($view->editor && $db->get_site_setting('use_admin_wysiwyg') == 'TinyMCE') {
			$gZip = ($db->get_site_setting('use_wysiwyg_compression'))? '_gzip':'';
			$jsScripts[] = "../js/tiny_mce/tiny_mce{$gZip}.js";
		}
...
class menu_category {
	 ...
	 *  the header text, if the category has a category page.
	 * @param String image this is the image filename to use on the main admin page, and in the header for the admin category page
...
class menu_page {
	...
	public static function addPage ($index, $parent, $title, $image, $filename, $classname, $type = 'main_page', $replace_existing=false){
		...
		$page->parent = $parent;
		$page->title = $title;
		$page->image = ($image=='') ? 'admin_images/dummy_image_change_in_admin_page_loader.php.gif" title="Image not configured.  Please set in Admin.class.php."' : "admin_images/$image";//if image is left blank, set it to a default one
	...
	public static function addonAddPage($index, $parent, $title, $addon_name, $image='', $type='main_page', $replace_existing=false){
		...
		$page->parent = $parent;
		$page->title = $title;
		$image = ($image=='') ? 'admin_images/menu_addons.gif' : "../addons/$addon_name/$image";
		$page->image = $image;
