{* 7.0.3-342-g15a09e9 *}
{if $info->icon_image}
<div style="float: right; padding-top: 10px;"><img src="../addons/{$info->name}/{$info->icon_image}" alt='icon' /></div>
{/if}
<strong>Addon Name: </strong>{$info->title}{if $info->info_url} [ <a href="{$info->info_url}">More Info</a> ]{/if}<br />
<strong>Author: </strong>{$info->author}{if $info->author_url} [ <a href="{$info->author_url}">Author's Site</a> ]{/if}<br />
{if $info->auth_tag}
...
{elseif $info_db.version && $info->upgrade_url}
