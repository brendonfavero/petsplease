...
<div class='row_color{cycle values="1,2"}'>
	...
	<div class='rightColumn'>
		<select id="gateway_dropdown" name="{$payment_type}[merchant_type]" {literal}onclick="if(this.value==1){document.getElementById('anet').style.display='block';document.getElementById('paytrace').style.display='none';document.getElementById('sim_radio').disabled=false;} else {document.getElementById('anet').style.display='none';document.getElementById('paytrace').style.display='block';document.getElementById('sim_radio').disabled=true;document.getElementById('aim_radio').checked=true;}"{/literal}>
			<option value="1" {if $values.merchant_type == 1}selected="selected"{/if}>Authorize.net</option>
			<option value="5" {if $values.merchant_type == 5}selected="selected"{/if}>Paytrace (AIM Method only)</option>
		</select>
	</div>
	<div class='clearColumn'></div>
</div>
