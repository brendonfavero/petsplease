...
class CategoriesManage
{
	public function display_category_config ()
	{
		...
		foreach ($values as $row) {
			//get listing count
			$row['listing_count'] = $db->GetOne("SELECT COUNT(*) FROM ".geoTables::listing_categories." WHERE `category`=?", array($row['category_id']));
	...
	public function update_category_create ()
	{
		...
		//insert it in there
		$result = $db->Execute("INSERT INTO ".geoTables::categories_table." SET `parent_id`=?, `level`=?, `enabled`=?, `which_head_html`=?, `display_order`=?",
		...
		foreach ($langs as $lang) {
			...
			$result = $db->Execute("INSERT INTO ".geoTables::categories_languages_table." SET `category_id`=?, `language_id`=?, `category_name`=?, `category_image`=?, `description`=?, `head_html`=?", array($cat_id, $lang['language_id'], $name, $image, $description, $head_html));
			if (!$result) {
	...
	public function update_category_edit ()
	{
		...
		$enabled = (isset($_POST['enabled']) && $_POST['enabled'])? 'yes' : 'no';
		//insert it in there
		$result = $db->Execute("UPDATE ".geoTables::categories_table." SET `level`=?, `enabled`=?, `display_order`=?,
				`which_head_html`=? ,
				`front_page_display` = ?
				WHERE `category_id`=?",
		...
		foreach ($langs as $lang) {
			...
			$result = $db->Execute("INSERT INTO ".geoTables::categories_languages_table." SET `category_id`=?, `language_id`=?, `category_name`=?, `category_image`=?, `description`=?, `head_html`=?", array($category_id, $lang['language_id'], $name, $image, $description, $head_html));
			if (!$result) {
	...
	public function update_category_create_bulk ()
	{
		...
		foreach ($names as $name) {
			...
			//insert it in there
			$result = $db->Execute("INSERT INTO ".geoTables::categories_table." SET `parent_id`=?, `level`=?, `enabled`=?, `display_order`=?",
	...
	public function update_category_durations_delete ()
	{
		...
		return $db->Execute("DELETE FROM ".geoTables::price_plan_lengths_table." WHERE `category_id`=? AND `length_id`=? AND `price_plan_id`=0",
	...
	private function copyCategory ($value_id, $parent)
	{
		...
		$result = $db->Execute("INSERT INTO ".geoTables::categories_table." SET ".implode(', ',$parts),
				$query_data);
		...
		foreach ($langs as $lang) {
			$row = $db->GetRow("SELECT * FROM ".geoTables::categories_languages_table." WHERE `category_id`=? AND `language_id`=?",
