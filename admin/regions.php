...
class RegionsManagement
{
	...
	public function update_region_create ()
	{
		...
		//insert it in there
		$result = $db->Execute("INSERT INTO ".geoTables::region." SET `parent`=?, `level`=?, `enabled`=?, `unique_name`=?, `display_order`=?,
	...
	public function update_region_edit ()
	{
		...
		//insert it in there
		$result = $db->Execute("UPDATE ".geoTables::region." SET `level`=?, `enabled`=?, `unique_name`=?, `display_order`=?,
	...
	public function update_region_edit_bulk ()
	{
		...
		if ($unique_use) {
			...
			if ($unique_use==='clear') {
				$set[] = "`unique_name`=''";
				$unique_use = false;
			} else if ($unique_use === 'abbreviation' || $unique_use==='auto_set' || (int)$unique_use > 0) {
				$unique_pre = trim($_POST['unique_pre']);
				$unique_post = trim($_POST['unique_post']);
			}
		}
	...
	public function update_region_move ()
	{
		...
		if ($parent == $new_parent) {
			geoAdmin::m("That is the same place that is already is!",geoAdmin::NOTICE);
			return false;
		}
