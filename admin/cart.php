...
class AdminCart
{
	...
	public function update_admin_cart_swap ()
	{
		...
		if (!$toCart || !$toOrderId) {
			//make sure the "to" cart exists
			//if it does not, create it (and the order)
			$useAdmin = ($direction=='to')? $adminId : 0;
			$cartId = (int)geoCart::createNewCart($this->userId, $useAdmin, 0, false);
			if (!$cartId) {
				geoAdmin::m("Error when attempting to create cart session, cannot swap items.", geoAdmin::ERROR);
				return false;
			}
			$toCart = $this->db->GetArray("SELECT * FROM ".geoTables::cart." WHERE `id`=$cartId");
			$toOrderId = (int)$toCart['order'];
		}
		if (!$toCart || !$toOrderId) {
		...
		} else if ($direction=='from' && strpos($fromCart['step'],':')!==false && !in_array($fromCart['main_type'], $skipTypes)) {
	...
	public function display_admin_cart_select_user ()
	{
		...
		} else if (isset($_GET['cart_deleted'])) {
