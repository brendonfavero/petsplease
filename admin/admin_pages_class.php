...
//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	function updateSurroundField(){
		...
		if (in_array($page_id,$this->cat_nav_mods)) {
		...
		} else if (in_array($page_id,$this->main_cat_nav_mods)) {
			//category navigation module
			...
			$this->body.='<tr class="'.$this->get_row_color().'"><td align="right" width="50%" class="medium_font"><strong>Display Subcategories (as links):</strong>'.$this->show_tooltip(1,1).'</td><td class="medium_font"><input name="c[module_display_sub_category_nav_links]" value="1"'.$yes_check.' type="radio" id="sub_cat_on"> yes<br />' .
					'<input name="c[module_display_sub_category_nav_links]" value="0"'.$no_check.' type="radio"> no</td></tr>';
		} else if (in_array($page_id,$this->filter_dropdown_mods)) {
			//filters dropdown
		...
		} else if (in_array($page_id,$this->display_username_mods)) {
		...
		} else if (in_array($page_id,$this->search_mods)) {
		...
		} else if (in_array($page_id,$this->zip_browse_mods)) {
			//display zip browsing module
			//no settings
		} else if (in_array($page_id,$this->state_browse_mods)) {
			//display state filter module
			//no settings
		} else if (in_array($page_id,$this->featured_listings_mods)) {
			...
			*/
		} else if (in_array($page_id,$this->fixed_cat_nav_mods)) {
		...
		} else if (in_array($page_id,$this->hottest)) {
			if (geoMaster::is('classifieds') && geoMaster::is('auctions')) {
				$this->input_module_display_type_listing($page);
			}
			...
			if ( geoPC::is_ent() )
				$this->input_module_display_optional_fields($page);
			*/
		} else if (in_array($page_id,$this->users)) {
			//user modules?  no settings.
		} else if (in_array($page_id, $this->my_account_links)) {
		...
		if (!$language_result) {
			...
			return false;
		} else if ($language_result->RecordCount() > 0) {
			$this->row_count = 0;
			while ($show_language = $language_result->FetchRow()) {
...
//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	public function module_display_fields_link ($activeTab = 'modules')
	{
		$this->body .= "
			<tr class=".$this->get_row_color().">
				<td align=right width=50% class=medium_font>
					<strong>Edit Which Fields are Displayed:</strong>
				</td>
				<td class=medium_font>
					Edit on <a href='index.php?page=fields_to_use&amp;activeTab={$activeTab}'>Listing Setup &gt; Fields to Use</a>
				</td>
			</tr>";
		$this->row_count++;
	}
	...
	function input_title_module_text($db,$page_id,$page)
	{
		...
		$this->body .= "
			<tr class=".$this->get_row_color().">
				...
				<td width=50% valign=top class=medium_font>
					<input type=text name=c[title_module_text] size=50 value=\"".stripslashes(urldecode($title_result["title_module_text"]))."\">
				</td>
			</tr>";
		if ($this->modules_debug)
		...
		$this->body .= "
			<tr class=".$this->get_row_color().">
				...
				<td width=50% valign=top class=medium_font>
					<input type=text name=c[title_module_home_text] size=50 value=\"".stripslashes(urldecode($this->messages[2462]))."\">
				</td>
			</tr>";
		if ($this->modules_debug)
