...
foreach ($versions as $version => $info) {
	if (isset($info['changelog'])) {
		...
		?>
		<li>
			<a href="<?php echo $info['changelog']; ?>"><?php echo $version;?><?php if ($high){ echo ' - '.$high; }?></a>
		</li>
		<?php
		//reset high
		$high = '';
	} else if (!$high && !isset($info['norelease']) && strpos($version, 'beta')===false && strpos($version, 'rc')===false) {
		$high = $version;
	}
