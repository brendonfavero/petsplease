<?php
//_samples/session.destroy_client.php
/**************************************************************************
...
//Sample API client, that uses the core.session.destroy api call to kill a session
//and log the user off, if the session is associated with a user.  As long as the
//session id sent has string length of 32, the api call will return true, it does
//not check to see if the session exists yet.
...
//The session id, as returned by a previous call to core.session.init
...
if (!$client->query('core.session.destroy',$data)){
