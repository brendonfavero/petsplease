...
class CLASSES_AJAXController_UploadAdlogo extends classes_AJAX {	
	...
	public function deleteImage ()
	{
		...
		$max = $data['maxSlots'] = adlogoOrderItem::getMaxImages();
		
		$imagesCaptured = adlogoOrderItem::getImagesCaptured('cart',true);
		if (!isset($imagesCaptured[$imageSlot])) {
			return $this->_returnError($this->messages[500681]);
		}
		//delete the image
		$removeResult = adlogoOrderItem::removeImage($imagesCaptured[$imageSlot]['id'], $imageSlot);
		if (!$removeResult) {
		...
		adlogoOrderItem::setImagesCaptured($newCaptured);
	...
	public function uploadImage ($data)
	{
		...
		if (isset($_POST['editImage'], $_POST['editImageSlot']) && $_POST['editImage']) {
			...
			if (isset($_FILES) && count($_FILES) > 0) {
				...
				$removeResult = adlogoOrderItem::removeImage($imagesCaptured[$slotNum]['id'], $slotNum);
				if (!$removeResult) {
		...
		if ($doNormalProcessing) {
			...
			geoOrderItem::callUpdate('mediaCheckVars',null,'adlogo');
			if (!$cart->errors) {
				geoOrderItem::callUpdate('mediaProcess',null,'adlogo');
			}
			
			if ($cart->errors > 0) {
				//oops! return error
				$msg = $cart->getErrorMsg('adlogo');
				if (!$msg) {
	...
	public function imageSlotContents ($skipInit = false)
	{
		...
		if ($latestCaptured) {
			...
			$imageData = adlogoOrderItem::getImgsData($latestCaptured, $maxW, $maxH);
			
			$cost = false;
			
			if (geoMaster::is('site_fees')) {
				$imageInfo = adlogoOrderItem::getImageData();
			...
			$tpl->assign($tpl_vars);
			$data['imagesDisplay'] = $tpl->fetch('adlogo/image_box.tpl');
		} else {
	...
	private function _validateCartStep()
	{
		...
		$validItems = adlogoOrderItem::getParentTypesFor('images');
		$validItems[] = 'adlogo'; //images would be the item if they clicked on edit button in cart.
	...
	private function _returnNewImages ($data = array(), $imagesCaptured = null)
	{
		...
		$data['uploadImageBox'] = geoOrderItem::callDisplay('mediaDisplay','justImageSlots','','adlogo');
