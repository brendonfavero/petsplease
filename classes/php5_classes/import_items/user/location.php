...
class locationImportItem extends geoImportItem
{
	...
	protected final function _updateDB($value, $groupId)
	{
		...
		if($levels['city'] && $regions[$levels['city']]) {
			geoImport::$tableChanges['userdata']['location_city'] = " `city` = '".geoRegion::getNameForRegion($regions[$levels['city']])."' ";
		}
		if($levels['state'] && $regions[$levels['state']]) {
			geoImport::$tableChanges['userdata']['location_state'] = " `state` = '".geoRegion::getNameForRegion($regions[$levels['state']])."' ";
		}
		if($levels['country'] && $regions[$levels['country']]) {
			geoImport::$tableChanges['userdata']['location_country'] = " `country` = '".geoRegion::getNameForRegion($regions[$levels['country']])."' ";
		}
		return true;
	}
