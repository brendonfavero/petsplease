...
/**
 * This class is responsible for a lot of the image processing specific to the
 * extra images stuff, along with other aspects like generating the image tag 
 * for thumbnails.
 * 
 * @package System
 * @since Version 4.0.4
...
class ppExtraImage
{
	...
	/**
	 * gets an instance of ppExtraImage using the Singleton method.
	 *
	 * @return ppExtraImage
	 */
	...
	public function copyImages ($id)
	{
		...
		$sql = "SELECT * FROM petsplease_classifieds_extraimages_urls WHERE `classified_id` = ? ORDER BY `display_order` ASC";
		...
		while($image_urls = $image_urls_result->FetchRow())
		{
			...
			$sql = "INSERT INTO petsplease_classifieds_extraimages_urls (".implode(', ',$first_part).") values (".implode(', ',$second_part).")";
	 ...
	 */
	public static function display_thumbnail ($classified_id,$max_width=0,$max_height=0,$skip_table_data=0,$affiliate_id=0,$aff_type='aff',$force_full_image=0)
	{
		...
		$sql = "select * from petsplease_classifieds_extraimages_urls where classified_id = ".(int)$classified_id." order by display_order asc, image_id desc limit 1";
		...
		if (!$image_url_result) {
			...
			$dims = ppExtraImage::getScaledSize($show_image_url->IMAGE_WIDTH, $show_image_url->IMAGE_HEIGHT, $current_max_width, $current_max_height, true);
			...
			$tpl_vars['imgTag'] = ppExtraImage::display_image($url, $width, $height,$show_image_url->MIME_TYPE,1,$alt_data);
		}
		$tpl->assign($tpl_vars);
	 ...
	 */
	public static function display_image ($url, $width=0, $height=0, $mime_type=0, $icon=0, $alt_data='')
	{
		$db = DataAccess::getInstance();
		$debug_display_image = 0;
		
		$this_copy =& $this;
		$overload = geoAddon::triggerDisplay('overload_Site_display_image', array ('url'=>$url, 'width' => $width, 'height' => $height,'mime_type' => $mime_type, 'icon' => $icon, 'this' => $this_copy), geoAddon::OVERLOAD);
		...
		if(!$mime_type) {
			...
			if((strpos($mime_type, "image/") === false) && (strlen(trim($mime_type)) > 0)) {
				...
				$sql_query = "select file.mime_type, icon_to_use from ".geoTables::file_types_table." as file, petsplease_classifieds_extraimages_urls as image where file.mime_type = image.mime_type and image_url like \"".$url."\" limit 1";
				...
				if(strpos($mime_type, "image/") === false) {
					$iconLocation = geoTemplate::getUrl('',$icon['icon_to_use']);
		...
		switch ($mime_type) {
			case 'x-shockwave-flash':
				if (!$icon) {
					$tpl = new geoTemplate('system', 'classes');
					$tpl->assign (array( 'width' => $width, 'height' => $height, 'url' => $url));
					$return = $tpl->fetch('Image/display_flash.tpl');
					return geoAddon::triggerDisplay('filter_display_image',$return,geoAddon::FILTER);
				} else {
					// Get the icon out of the database
					$sql_query = "select file.mime_type, icon_to_use from ".geoTables::file_types_table." as file, petsplease_classifieds_extraimages_urls as image where file.mime_type = image.mime_type and image_url like \"".$url."\" limit 1";
					...
					if(strpos($mime_type, "image/") === false) {
						...
						$tpl = new geoTemplate('system','classes');
						$tpl->assign (array( 'src' => $iconLocation,
						'width' => $size[0],
						'height' => $size[1],
						'alt' => $alt_data));
						$return = $tpl->fetch('Image/display_image.tpl');
					...
					//FIXME: If it gets here, what should it return?  Currently it would return null...
				}
				break;

			default:
				$tpl = new geoTemplate('system','classes');
				$tpl->assign(array( 'src' => $url,
				'width' => $width,
				'height' => $height,
				'alt' => $alt_data));
				$return = $tpl->fetch('Image/display_image.tpl');
	...
	public static function fixDuplicates ($listingId)
	{
		...
		$sql = "SELECT `image_id`, `display_order` FROM petsplease_classifieds_extraimages_urls WHERE `classified_id` = ? ORDER BY `display_order`, `image_id`";
	...
	public static function getRemoteDims($image_id)
	{
		...
		$db = DataAccess::getInstance();
		$url = $db->GetOne("SELECT `image_url` FROM petsplease_classifieds_extraimages_urls WHERE `image_id` = ?", array($image_id));
		if(!$url) {
			//couldn't get image
			return false;
		}
		...
		$update = "UPDATE petsplease_classifieds_extraimages_urls SET `image_width` = ?, `image_height` = ?, `original_image_width` = ?, `original_image_height` = ?, `mime_type` = ? WHERE `image_id` = ?";
