		...
		if($this->_importType === self::USER_IMPORTTYPE) {
			if(self::$tableChanges['userdata']) {
				...
				if(!$db->Execute($updateUserdata)) {
					trigger_error('ERROR IMPORT: userdata query failed');
					return false;
				}
			}
			...
			if(self::$tableChanges['ugpp']) {
				$updateGroupsPlans = "UPDATE `geodesic_userdata` SET ";
				$sqls = array();
				...
				if(!$db->Execute($updateGroupsPlans)) {
					trigger_error('ERROR IMPORT: ugpp query failed');
					return false;
				}
			}
		}
