...
class geoRegion
{
	 ...
	 * @return String
	 */
	public static function regionSelector($fieldName, $prevalue=array(), $maxLevel=false, $required=false)
	{
		if($maxLevel === 0) {
			//maxLevel IS set, and IS 0 -- there are no levels enabled! show nothing!
			return false;
		}
		$tpl_vars = self::getRegionsFromParent($fieldName, 0, $prevalue, $maxLevel);
		if($tpl_vars === false) {
	...
	public static function billingCountrySelector($name, $prevalue=false)
	{
		...
		$tpl->assign('is_a', (defined('IN_ADMIN') ? 1:0));
	 ...
	 * @return Array
	 */
	public static function getRegionsFromParent($fieldName, $parent=0, $prevalue=array(), $maxLevel=false)
	{
		$db = DataAccess::getInstance();
