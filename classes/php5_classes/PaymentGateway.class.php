...
class geoPaymentGateway
{
	...
	final public static function loadGateways ($dirname = '', $force_serialize = true)
	{
		...
		while ($filename = readdir($dir)){
			if ($filename !='.' && $filename != '..' && $filename != 'seller_buyer' && strpos($filename,'_') !== 0 && strpos($filename,'.php') !== false && file_exists($dirname.$filename)){
	...
	protected static function _success($order, $transaction, $gateway, $skipDisplay = false)
	{
		...
		$order->save();
		if (!$skipDisplay) {
			self::_successFailurePage(true, $order->getStatus());
		}
	}
	 ...
	 */
	protected static function _successFailurePage($success=false, $status = 'pending', $render = true, $invoice = null)
	{	
