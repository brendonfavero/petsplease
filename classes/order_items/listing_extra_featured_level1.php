...
class listing_extra_featured_level1OrderItem extends geoOrderItem {
	var $defaultProcessOrder = 35;
	protected $type = 'listing_extra_featured_level1';
	const type = 'listing_extra_featured_level1';
	/**
	...
	public static function geoCart_initSteps_addOtherDetails(){
		...
		if (!$db->get_site_setting('use_featured_feature')) {
	...
	public static function geoCart_other_detailsCheckVars($c_var = false){
		...
		if (isset($_POST['c']) || $c_var){
			$use_featured_ad = ((isset($_POST['c']['featured_ad']) && $_POST['c']['featured_ad'])? 1: 0);
			$use_featured_ad = (($use_featured_ad || (isset($c_var['featured_ad']) && $c_var['featured_ad']))? 1: 0);
			
			$cart->site->session_variables['featured_ad'] = $use_featured_ad;
			...
			//get current attached featured_ad, if exists..
			$items = $cart->order->getItem('listing_extra_featured_level1');
			$order_item = false;
			if (is_array($items)){
			...
			if (!$use_featured_ad){
				...
				if (!$order_item){
					...
					trigger_error('DEBUG CART: Adding featured_level1: <pre>'.print_r($order_item,1).'</pre>');
				} else {
					trigger_error('DEBUG CART: featured_ad already attached: <pre>'.print_r($order_item,1).'</pre>');
					$cart->order->addItem($order_item);
				}
				//get the price for featured_ad
				$cost = (!geoMaster::is('site_fees'))? 0: $cart->price_plan['featured_ad_price'];
				$order_item->setCost($cost);
				$order_item->setCreated($cart->order->getCreated());
				
				//set details specific to featured_ad
			...
			trigger_error('DEBUG CART: featured_ad: '.$cart->site->session_variables['featured_ad']);
		}
		
		//but children might, get steps from children as well.
		$children = geoOrderItem::getChildrenTypes('listing_extra_featured_level1');
	...
	public function getDisplayDetails ($inCart,$inEmail=false)
	{
		...
		if ($parent && $parent->getType() === 'listing_renew_upgrade') {
			$return = $parent->checkNoDowngrade($return, 'featured_ad');
		}
		return $return;
	}
	...
	public static function geoCart_other_detailsDisplay (){
		$cart = geoCart::getInstance();
		if (!in_array ($cart->main_type,geoOrderItem::getParentTypesFor(self::type)) || !$cart->db->get_site_setting('use_featured_feature'))
		{
			//do not show thingy for bolding
			return '';
		}
		if (isset($cart->item->renew_upgrade) && $cart->item->renew_upgrade == listing_renew_upgradeOrderItem::upgrade){
			...
			if ($cart->site->parent_session_variables['featured_ad']){
				//already exists on parent, do not allow adding
				return '';
			}
		}
		$cart->setPricePlan($cart->item->getPricePlan(),$cart->item->getCategory());
		if (geoPC::is_ent() && !$cart->price_plan['use_featured_ads']){
			//none for this price plan
			return '';
		}
		$price = (!geoMaster::is('site_fees'))? 0: $cart->price_plan['featured_ad_price'];
		$return = array (
			'checkbox_name' => 'c[featured_ad]', //no checkbox display
			'title' => geoString::fromDB($cart->site->messages[217]),
			'display_help_link' => $cart->site->display_help_link(218),//if 0, will display no help icon thingy
			'price_display' => geoString::displayPrice($price, false, false, 'cart'),
			'checked' => (($cart->site->session_variables['featured_ad']) ? "checked=\"checked\" " : "")
		);
		if ($cart->item && $cart->item->getType()==='listing_renew_upgrade') {
			...
			$return = $cart->item->checkNoDowngrade($return, 'featured_ad');
		}
		return $return;
	}
	...
	public static function copyListing(){
		...
		if($cart->site->session_variables['featured_ad'])
		{
			if (!in_array ($cart->main_type,geoOrderItem::getParentTypesFor(self::type)) || !$cart->db->get_site_setting('use_featured_ad_feature'))
			{
				//do not show
				return '';
			}
			$cart->setPricePlan($cart->item->getPricePlan(),$cart->item->getCategory());
			if (geoPC::is_ent() && !$cart->price_plan['use_featured_ad']){
