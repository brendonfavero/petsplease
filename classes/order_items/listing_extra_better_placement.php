<?php
//order_items/listing_extra_better_placement.php
/**************************************************************************
...
class listing_extra_better_placementOrderItem extends geoOrderItem {
	var $defaultProcessOrder = 32;
	protected $type = 'listing_extra_better_placement';
	const type = 'listing_extra_better_placement';
	...
	public static function geoCart_initSteps_addOtherDetails(){
		...
		if (!$db->get_site_setting('use_better_placement_feature')) {
	...
	public function getDisplayDetails ($inCart,$inEmail=false)
	{
		...
		if ($parent && $parent->getType() === 'listing_renew_upgrade') {
			$return = $parent->checkNoDowngrade($return, 'better_placement');
		}
		return $return;
	}
	...
	public static function geoCart_other_detailsCheckVars($c_var = false){
		...
		if (isset($_POST['c']) || $c_var){
			$use_better_placement = ((isset($_POST['c']['better_placement']) && $_POST['c']['better_placement'])? 1: 0);
			$use_better_placement = (($use_better_placement || (isset($c_var['better_placement']) && $c_var['better_placement']))? 1: 0);
			$cart->site->session_variables['better_placement'] = $use_better_placement;
			...
			$items = $cart->order->getItem('listing_extra_better_placement');
			$order_item = false;
			if (is_array($items)){
				foreach ($items as $i => $val){
			...
			if (!$use_better_placement){
				...
				if (!$order_item){
					$order_item = new listing_extra_better_placementOrderItem;
					$order_item->setType('listing_extra_better_placement');
					...
					trigger_error('DEBUG CART: Adding better_placement: <pre>'.print_r($order_item,1).'</pre>');
				} else {
					trigger_error('DEBUG CART: better_placement already attached: <pre>'.print_r($order_item,1).'</pre>');
					$cart->order->addItem($order_item);
				}
				//get the price for better_placement
				$cost = $order_item->_getbetter_placementCost();
				$order_item->setCost($cost);
				$order_item->setCreated($cart->order->getCreated());
				
				//set details specific to better_placement
			...
			trigger_error('DEBUG CART: better_placement: '.$cart->site->session_variables['better_placement']);
		}
		if (!$c_var){
			//but children might, get steps from children as well.
			$children = geoOrderItem::getChildrenTypes('listing_extra_better_placement');
	...
	public static function geoCart_other_detailsDisplay (){
		...
		if (!in_array ($cart->main_type,geoOrderItem::getParentTypesFor(self::type)) || !$cart->db->get_site_setting('use_better_placement_feature'))
		{
			//do not show
			return '';
		}
		...
		if (isset($cart->item->renew_upgrade) && $cart->item->renew_upgrade == listing_renew_upgradeOrderItem::upgrade){
			...
			if ($cart->site->parent_session_variables['better_placement']){
				//already exists on parent, do not allow adding
				return '';
			}
		}
		$cart->setPricePlan($cart->item->getPricePlan(),$cart->item->getCategory());
		if (geoPC::is_ent() && !$cart->price_plan['use_better_placement']){
			//turned off at the price plan level, don't show
			return '';
		}
		$price = (!geoMaster::is('site_fees'))? 0: $cart->price_plan['better_placement_charge'];
		$return = array (
			'checkbox_name' => 'c[better_placement]', //no checkbox display
			'title' => geoString::fromDB($cart->site->messages[215]),
			'display_help_link' => $cart->site->display_help_link(216),//if 0, will display no help icon thingy
			'price_display' => geoString::displayPrice($price, false, false, 'cart'),
		);
		
		$return['checked'] = ($cart->site->session_variables['better_placement']) ? "checked=\"checked\" " : "";
		...
		if ($cart->item && $cart->item->getType()==='listing_renew_upgrade') {
			...
			$return = $cart->item->checkNoDowngrade($return, 'better_placement');
		}
	...
	function _getbetter_placementCost(){
		$cart = geoCart::getInstance();
		if (!$cart->price_plan) {
		...
		if ($cart->db->get_site_setting('use_better_placement_feature') && $cart->site->session_variables["better_placement"]) {
			return $cart->price_plan['better_placement_charge'];
		}
		return 0;
	}
	...
	public static function geoCart_deleteProcess(){
		...
		if (is_object($parent)){
			...
			$session_vars['better_placement'] = 0;
	...
	public static function copyListing(){
		...
		if($cart->site->session_variables['better_placement'])
		{
			if (!in_array ($cart->main_type,geoOrderItem::getParentTypesFor(self::type)) || !$cart->db->get_site_setting('use_better_placement_feature'))
			{
				//do not show
	...
	public static function listing_insertListingFromSessionVars ($vars)
	{
		...
		$old = ($listing->better_placement > 0);
		
		$new = (isset($session_variables['better_placement']) && $session_variables['better_placement'] > 0);
		
		//change better placement to off or on
		if (!$new) {
			//just turn it off
			$better_placement = 0;
		} else {
			//turning better placement on
			...
			if ($listing->live && $db->get_site_setting($prefix.'rotate')) {
				//using rotation, so it's a little more complicated than setting to 1
				
				if ($db->get_site_setting($prefix.'perCategory')) {
					//get category ID, we rotate based on category
					$categoryId = (int)$session_variables['category'];
					if (!$categoryId) {
						$categoryId = (int)$listing->category;
					}
					$booth = (int)$db->GetOne("SELECT MAX(`better_placement`) FROM ".geoTables::classifieds_table." WHERE `live`=1 AND `category`={$categoryId}");
				} else {
					$booth = (int)$db->GetOne("SELECT MAX(`better_placement`) FROM ".geoTables::classifieds_table." WHERE `live`=1");
				}
				$booth += 1;
				$booth = ($booth >= 2)? $booth : 2;
				
				//set better placement value to be 1 above the current max value
				//for better placement, so it shows up at top.
				$better_placement = $booth;
			}
		}
		$listing->better_placement = $better_placement;
	}
}