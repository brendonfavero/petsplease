...
class Browse_ads extends geoBrowse {
	...
	function browseCategory ($browse_type=0)
	{
		...
		if ($this->db->get_site_setting('display_sub_category_ads')) {
			...
			$this->in_statement = " = {$this->site_category}";
		}
		...
		$query->where("$classTable.`category` ".$this->in_statement, 'category')
		...
		} else if (isset($orderBy["category"]) && $orderBy["category"] && $useCategorySettings && geoPC::is_ent()) {
		...
		*/
		...
		$show_auctions = (!geoMaster::is('auctions') || $opt_type == 1 || $config_type == 1) ? false : true;
		$show_classifieds = (!geoMaster::is('classifieds') || $opt_type == 2 || $opt_type == 4 || $config_type == 2) ? false : true;
		...
		if ($show_classifieds && $show_auctions) {
			...
			} else if ($this->db->get_site_setting('display_all_tab_browsing')) {
				//for the all tab
