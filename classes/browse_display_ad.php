...
//###########################################################
	...
	public function display_classified($id=0,$return=false,$preview = false, $autoDisplay = true, $session_vars = null, $printFriendly = false)
	{
		...
		//when on..
		if ($show["live"] && geoMaster::is('auctions') && $show['item_type']==2 && $time <= geoUtil::time() && ($show['ends']>geoUtil::time() || $show["delayed_start"])) {
			if ($browser_user_id) {
		...
		if ($preview != 'preview_only' && $show["item_type"] == 2 && $show["ends"] <= geoUtil::time() && $show["delayed_start"] == 0) {
		...
		if ($show["item_type"] == 2) {
			...
			if ($show["auction_type"]!=2 && $show['buy_now']>0) {
				...
				if ($show['buy_now_only']) {
					...
					$show_buy_now = true;
				} else if ($show['current_bid']!=0 && $this->configuration_data['buy_now_reserve']==1 && !$reserve_met) {
