...
class authorizenetPaymentGateway extends _ccPaymentGateway{
	 ...
	 * @var string
	 */
	private static $_submitUrl = 'https://secure.authorize.net/gateway/transact.dll';
  ...
  <subscriptionId>$recurringId</subscriptionId>
	...
	public static function admin_display_payment_gateways (){
		$return = array (
			...
			'title' => 'CC - Authorize.net / Paytrace',//how it's displayed in admin
	...
	public function admin_custom_config (){
		...
		$tooltips['merchant_type'] = geoHTML::showTooltip('Choose a gateway', 'You may use either the Authorize.net or Paytrace gateway with these settings. Both gateways share many similar settings, so they are included on one page for convenience.');
	...
	public function admin_update_payment_gateways(){
		...
		if (isset($_POST['authorizenet']) && is_array($_POST['authorizenet']) && count($_POST['authorizenet']) > 0){
			...
			if (geoMaster::is('auctions')) {
				...
				if ($charge_final_fees && ($set['connection_type'] != 2 || $set['merchant_type'] != 1)) {
					geoAdmin::m('Must use AIM method, and authorize.net merchant, to be able to auto-charge final fees.', geoAdmin::NOTICE, true, 5);
	...
	public static function geoCart_payment_choicesProcess ()
	{
		...
		if ($values["connection_type"] == 1) {
			...
			$data = $values["merchant_login"] ."^". $sequence ."^". $currenttime ."^". $transaction->getAmount() ."^";
			...
			if ($result) {
				...
				//show success page
				self::_successFailurePage(true, $cart->order->getStatus(), true, $invoice);
			} else {
				//failed!
	...
	public static function auction_final_feesOrderItem_canAutoCharge ()
	{
		...
		//make sure it is aim method and authorize.net method:
		//authorize.net: merchant_type = 1
		//aim: connection_type = 2
		if ($gateway->get('merchant_type') != 1 || $gateway->get('connection_type') != 2) {
	...
	private static function _processAim ($info, $order, $authOnly = false, $recurring = false)
	{
		...
		if($values['merchant_type'] == 5){
			$post_url = "https://PayTrace.com/api/gateway.pay";
		} else {
			if ($gateway->get('testing_mode')) {
				//TESTING MODE
