...
class beanstreamPaymentGateway extends _ccPaymentGateway{
	...
	public static function geoCart_payment_choicesProcess(){
		...
		if($info['firstname']) $params['billing']['name'] = $info['firstname'];
		if($info['lastname']) $params['billing']['name'] .= ' '.$info['lastname'];
		if($info['address']) $params['billing']['address_line1'] = $info['address'];
		if($info['address_2']) $params['billing']['address_line2'] = $info['address_2'];
		if($info['city']) $params['billing']['city'] = $info['city'];
		if($info['state']) $params['billing']['province'] = $info['state'];
		if($info['country']) $params['billing']['country'] = $info['country'];
		if($info['zip']) $params['billing']['postal_code'] = $info['zip'];
		if($info['email']) $params['billing']['email_address'] = $info['email'];
		if($info['phone']) $params['billing']['phone_number'] = $info['phone'];
