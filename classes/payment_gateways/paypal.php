...
class paypalPaymentGateway extends geoPaymentGateway
{
	...
	protected $_paypal_cancel_instructions = 'https://www.paypal.com/helpcenter/main.jsp?cmd=_help-ext&t=solutionTab&solutionId=27715';
	...
	public function admin_update_payment_gateways ()
	{
		if (isset($_POST['paypal']) && is_array($_POST['paypal']) && count($_POST['paypal']) > 0) {
			...
			} else if (!geoString::isEmail($paypal_id)) {
			...
			} else if (!geoString::isEmail($micro_id)) {
	...
	public static function geoCart_payment_choicesProcess ()
	{
		...
		if ($recurring) $recurring->setSecondaryId($unique_transaction);
		...
		} else if (strlen(trim($gateway->get("paypal_item_label"))) > 0) {
		...
		$return_url = str_replace($cart->db->get_site_setting("classifieds_file_name"), $fileName,$cart->db->get_site_setting("classifieds_url"));
		
		$paypal_url = "https://$host/cgi-bin/webscr?";
		$paypal_url .= "receiver_email=".urlencode($paypal_id);
		$paypal_url .= "&return=".urlencode($cart->db->get_site_setting("classifieds_url"));
		...
		if ($recurring) {
			...
			if ($days > 90) {
				...
				if ($days >= 365 && $days%365 == 0) {
					...
					$t3 = 'Y';
				} else if ($days%7 == 0 && $days/7 <= 52) {
					//translates cleanly into weeks, use weeks
					$t3 = 'W';
				} else if ($days%30 == 0 && $days/30 <= 24) {
